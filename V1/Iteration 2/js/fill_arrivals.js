...
function updateArrivalsInterval(stopName, lineNumber, directionLetter, stopId) {
  function getTenNextArrivals() {
    const stopKey = Object.keys(arrivals).find((key) => {
      ...
      return (
        keySplitted[0] == lineNumber &&
        keySplitted[1] == directionLetter &&
        keySplitted[2] == stopId
      );
    });

    const currentTime = getFormattedCurrentTime();
    const sortedArrivals = arrivals[stopKey].sort(function (
      arrival1,
      arrival2
    ) {
    ...
    if (extra < 0) {
      return sortedArrivals
        .slice(startIndex, sortedArrivals.length)
  ...
  function showTenNextArrivals() {
    ...
    if (tenNextArrivals.length > 0) {
      ...
      document.getElementById("arrival-stop-name").innerHTML =
        stopName + " (" + stopId + ")";
      ...
      tenNextArrivals.forEach((arrival) => {
        ...
        td.textContent =
          arrival[0] + arrival[1] + " h " + arrival[2] + arrival[3];
        tr.appendChild(td);
