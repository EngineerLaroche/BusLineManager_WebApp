...
function displayMap() {
    ...
    if (getStops() != null) {
        ...
        // Mise à jour des positionnements des bus à intervalle régulier
...
function setMarkers() {
    ...
    if (mtl_map != null && stops != null) {
        ...
        for (var i = 0; i < stops.length; i++) {
            ...
            var popupContent = `<b>${stops[i].id} ${stops[i].name}</b>`;

            // Paremetres du marker
...
function openPopup(e) {
    ...
    // Recupere le popup en cours
    var popup = e.target.getPopup();
    var content = popup.getContent();
    var info = e.target.stopInfo;
    var tenArrivals = "";

    // Recupere l'horaire des arrivees
    ...
    function waitFirstMarkerClick() {
        if (firstMarkerClick) {
            setTimeout(function () { waitFirstMarkerClick(); }, 100);
        } else {
            ...
            if (arrivalTimes.length > 0)
                for (var i = 0; i < arrivalTimes.length; i++)
                    tenArrivals += "<br>> " + arrivalTimes[i];

            // Insere le contenu dans le popup
            popup.setContent(content + '<br>' + tenArrivals);
            popup.update();
        };
    }
}
...
async function setBusMarkers() {
    if (mtl_map != null) {
        try {
            ...
            const positions = await fetchBusPositions(markerLine, markerDirection, fetchBusPositionsCancellationToken);
            hideError();
            const stops = getStops();
