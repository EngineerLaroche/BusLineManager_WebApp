...
/********************************************
 * TABS HANDLER     (ONGLETS)
 * 
 * C'est ici qu'on gère les onglets.
 *
 * Lorsque l'utilisateur sélectionne un onglet,
 * le contenu de la fenêtre change.
 *
 * Dans le cas présent, les onglets sont pour
 * afficher l'horaire et la Carte.
 * 
 ********************************************/
jQuery(document).ready(function () {

    // Si l'utilisateur clique sur un onglet
    jQuery('.onglets .ongletsRef a').on('click', function (e) {

        // Affiche ou cache l'onglet selon la selection
        var ref = jQuery(this).attr('href');
        jQuery('.onglets ' + ref).show().siblings().hide();
        jQuery(this).parent('li').addClass('active').siblings().removeClass('active');
        e.preventDefault();

        // Réinitialise la carte avec le nouveau contenu
        carte.reset();

        // Pour initialiser les horaires des favoris au changement de Tab
		initfavoris();
    });
});