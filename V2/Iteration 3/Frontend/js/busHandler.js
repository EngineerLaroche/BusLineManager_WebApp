...
var BusHandler = function () {
    ...
    return new Promise(function (resolve, reject) {
      ...
      xhr.onload = function () {
        if (xhr.status >= 200 && xhr.status < 300) {
          ...
          busError(
            xhr,
            reject,
            "Position des bus - Requête impossible (code: " + xhr.status + ") !"
          );
        }
      };
      xhr.ontimeout = function () {
        busError(
          xhr,
          reject,
          "Position des bus - Requête expirée après: " + xhr.timeout + " ms"
        );
      };
  ...
  function updateBusMarkers() {
    ...
    busIntervalID = setInterval(function () {
      initBusMarkers();
      initFavoriBusMarkers();
    }, 10000);
  }
  ...
  async function initBusMarkers() {
    ...
    busMarkers.forEach((marker) => {
      carte.get().removeLayer(marker);
    });
    busMarkers = [];

    // Recupere la position des bus en appelant l'API
    busList = await requestBus(
      ligne.get().id,
      directionLetter(ligne.get().direction)
    );
    ...
    for (var i = 0; i < busList.length; i++) {
      ...
      const marker = L.marker([busList[i].lat, busList[i].lon], {
        icon: busIcon,
      })
        .bindPopup(busPopup)
        .addTo(carte.get());
