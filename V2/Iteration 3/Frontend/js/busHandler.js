...
var BusHandler = function () {
    ...
    ********************************************/
		...
		return new Promise(function (resolve, reject) {
			...
			xhr.onload = function () {
			  if (xhr.status >= 200 && xhr.status < 300) {
				...
				busError(xhr, reject, "Favori STM - Requête impossible (code: " + xhr.status + ") !");
			  }
			};
			xhr.ontimeout = function () {
			  busError(xhr, reject, "Favori STM - Requête expirée après: " + xhr.timeout + " ms");
			}
			xhr.onerror = function () {
				  if(xhr.readyState == 4){
					lineError(xhr, reject, "Favori STM - Requête a générée une erreur !\n Veuillez vérifier le url utilisé ou vos paramêtre utilisé.");
				  }else{
					lineError(xhr, reject, "Favori STM - Requête a générée une erreur !\n Veuillez vérifier votre connexion internet.");
				  }
			};
			xhr.onabort = function () {
			  busError(xhr, reject, "Favori STM - Requête annulée !");
			};
			xhr.timeout = 10000;
			xhr.send();
		});
    ...
    function updateBusMarkers() {
        ...
        busIntervalID = setInterval(function () { initBusMarkers(); initFavoriBusMarkers(); }, 10000);
    }
    ...
    async function initBusMarkers() {
        ...
        busList = await requestBus(ligne.get().id, directionLetter(ligne.get().direction));
    ...
    /********************************************
     * INIT FAVORI BUS MARKERS
     * Insere et affiche les markers des lignes Favori.
     ********************************************/
    async function initFavoriBusMarkers() {
        ...
        for (var j = 0; j < favoriStop.length; j++) {
			...
			// Recupere la position des bus en appelant l'API
			const busList = await requestBus(splitFavorite[0], splitFavorite[1]);
			...
			for (var i = 0; i < busList.length; i++) {
				...
				var stopCode = splitFavorite[0]+'-'+splitFavorite[1]+'-'+busList[i].next_stop;
				...
				if(favoriStop.includes(stopCode)) {
					// Associe le contenu du bus à son marker
					const marker = L.marker([busList[i].lat, busList[i].lon], { icon: busIcon })
						.bindPopup(busPopup)
						.addTo(carte.get());
