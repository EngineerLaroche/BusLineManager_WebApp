	...
	if (!(sessionStorage.getItem(url) == null)) {
        ...
        xhr.onload = function () {
          if (xhr.status >= 200 && xhr.status < 300) {
            ...
            lineError(xhr, reject, "Lignes STM - Requête impossible (code: " + xhr.status + ") !");
          }
        };
        xhr.ontimeout = function () {
          lineError(xhr, reject, "Lignes STM - Requête expirée après: " + xhr.timeout + " ms");
        }
        xhr.onerror = function () {
		  if(xhr.readyState == 4){
			lineError(xhr, reject, "Lignes STM - Requête a générée une erreur !\n Veuillez vérifier le url utilisé ou vos paramêtre utilisé.");
		  }else{
  ...
  ********************************************/
  ...
  async function initLines() {
    ...
    for (categories in linesMapping) {
      ...
      } else { // On récupère les lignes des catégories
        var busLines = lignes.filter(line =>
          line.category === linesMapping[categories]
        );
  ...
  ********************************************/
  function initDirection(line, directionSide) {
    ...
    directionRef.onclick = function () {
      ...
      document.getElementById("info-ligne").textContent = line.id + " " + line.name + " - " + line.direction;
      _ligne = line;
      arret.init(line);
    }
  ...
  return {
    init: function () { initLines(); },
    get: function () { return _ligne }
  }
}
